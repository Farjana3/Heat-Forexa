...
def run_correlation_analysis(
    ...
    corr_pearson.to_csv(
        os.path.join(output_dir, "correlation_matrix_pearson.csv")
    )

    corr_spearman.to_csv(
        os.path.join(output_dir, "correlation_matrix_spearman.csv")
    )
    ...
    plt.title(
        ...
        "(Temperature & Environmental Features)"
    )
...
def run_stationarity_tests(
    ...
    df_adf.to_csv(
        os.path.join(output_dir, "stationarity_results.csv"),
        index=False
    )
...
def run_granger_causality_analysis(
    ...
    for col in candidates:
        ...
        try:
            ...
            # Test whether col Granger-causes temperature
    ...
    df_granger.to_csv(
        os.path.join(output_dir, "granger_causality_results.csv"),
        index=False
    )

    # Identify selected exogenous variables
    # with significant Granger causality (p < 0.05)
    sig_vars = (
        df_granger[df_granger['p_value'] < 0.05]
        ['exogenous_variable']
        .unique()
        .tolist()
    )

    # Fallback to selected environmental variables
    if not sig_vars:
        sig_vars = [
            'temperature_2m_mean',
            'apparent_temperature_mean',
    ...
    with open(
        os.path.join(output_dir, "selected_exogenous_variables.json"),
        'w'
    ) as f:

        json.dump(
            {'selected_exogenous': sig_vars},
            f,
            indent=2
        )
    ...
    # ============================================================
    # Plot Granger Causality p-values
    # Matplotlib is used instead of seaborn barplot to avoid
    # hue/legend compatibility issues with current library versions.
    # ============================================================

    plt.figure(figsize=(10, 5))

    for lag in sorted(df_granger['lag'].unique()):
    ...
    plt.title(
        "Granger Causality Test P-Values across Lags "
        "(X -> FortyGuard Temperature)"
    )
